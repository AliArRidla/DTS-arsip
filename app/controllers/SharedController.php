...
class SharedController extends BaseController{
	...
	function pilih_surat_kategori_id_option_list(){
		$db = $this->GetModel();
		$sqltext = "SELECT id AS value, type_kategori AS label FROM kategori";
		$queryparams = null;
		$arr = $db->rawQuery($sqltext, $queryparams);
		return $arr;
	}

	function pilih_nama_kategori_id_option_list($val){
		$db = $this->GetModel();
		// $sqltext = $db->has("kategori");
		// $db->where("", $val);
		$sqltext = "SELECT id AS value, type_kategori AS label FROM kategori WHERE id = $val";		
		$queryparams = null;
		$arr = $db->rawQuery($sqltext, $queryparams);
		return $arr;
	}
